...
// // Update user with photo upload - FIXED VERSION
...
//     // Check if email is taken by another user (only if email is being changed)
...
//     // Parse location if it's a string
//     let parsedLocation = existingUser.location || {};
//     if (location) {
//       try {
//         parsedLocation = typeof location === 'string' ? JSON.parse(location) : location;
//       } catch (e) {
...
//       userHyperlink: userHyperlink !== undefined ? userHyperlink : existingUser.userHyperlink,
...
//         const oldPath = '.' + existingUser.profilePhoto;
...
router.get('/', async (req, res) => {
  try {
    const { role, search, page = 1, limit = 10, approved } = req.query;
    const query = {};
...
// Create new user with photo upload (Admin only)
router.post('/', upload.single('profilePhoto'), async (req, res) => {
  try {
    const { firstName, lastName, email, password, phone, role, company, location, isActive } = req.body;
    const adminId = req.headers['admin-id']; // Pass admin ID from frontend
    
    console.log('Creating user with data:', { 
      firstName, lastName, email, phone, role, company, location 
    });
    ...
    const user = new User({
      ...
      role: role || 'user',
      isActive: isActive !== undefined ? isActive : true,
      isApproved: false, // Users need approval
      company: company || '',
      location: parsedLocation || {},
      profilePhoto: req.file ? `/uploads/profiles/${req.file.filename}` : '',
    ...
    console.log('User created successfully. Needs approval:', userResponse.email);

    res.status(201).json({
      success: true,
      data: userResponse,
      message: 'User created successfully. They will need to be approved before they can login.'
    });
  } catch (error) {
    console.error('Error creating user:', error);
    res.status(500).json({
...
router.put('/:id', upload.single('profilePhoto'), async (req, res) => {
  try {
    ...
    const { firstName, lastName, email, password, phone, role, isActive, isApproved, company, location } = req.body;
...
router.get('/stats/pending-approval', async (req, res) => {
  try {
    const count = await User.countDocuments({ isApproved: false, role: 'user' });
    res.json({
      success: true,
