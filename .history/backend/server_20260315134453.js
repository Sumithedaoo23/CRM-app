...
// ============ SIMPLE FIX ADMIN ROUTE ============
// This will create/update admin with your credentials
app.get('/api/fix-admin-simple', async (req, res) => {
  try {
    const bcrypt = require('bcryptjs');
    
    // Your credentials
    const adminEmail = 'sumithedaoo07@gmail.com';
    const adminPassword = 'sumit123';
    
    console.log('🔧 Fixing admin with email:', adminEmail);
    ...
    if (user) {
      ...
      user.firstName = user.firstName || 'Sumit';
      user.lastName = user.lastName || 'Hedaoo';
      await user.save();
...
app.listen(PORT, () => {
  ...
  console.log(`   │  ├─ GET  /api/dashboard/admin`);
  console.log(`   │  └─ GET  /api/dashboard/user/:userId`);
  console.log('   └─ Admin Fix');
  console.log(`      └─ GET  /api/fix-admin-simple (Fix admin credentials)`);
  console.log('='.repeat(50) + '\n');
});