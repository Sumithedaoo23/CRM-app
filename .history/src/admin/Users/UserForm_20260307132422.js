...
const UserForm = () => {
  ...
  const generateDefaultCredentials = () => {
    ...
    const emailPrefix = formData.firstName && formData.lastName 
  ...
  const fetchUser = async () => {
    try {
      ...
      setFormData({
        ...
        role: user.role || 'user',
        isActive: user.isActive !== undefined ? user.isActive : true,
        userHyperlink: user.userHyperlink || `https://crm.com/user/${user._id?.slice(-6)}`,
        profilePhoto: null,
    ...
    React.createElement('div', {
      ...
      React.createElement('form', {
        ...
        <div style={{
          ...
          >{loading ? 'Saving...' : (isEditMode ? 'Update User' : 'Create User')}</button>
        </div>
      )
    ),
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '20px' }}>
          ...
          <h2 style={{
            ...
            color: '#111827',
            marginBottom: '4px'
          }}>User {isEditMode ? 'Updated' : 'Created'} Successfully!</h2>
        </div>

        {/* Profile Photo in Popup */}
        {savedUser.profilePhotoPreview && (
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: '13px', fontWeight: '600', color: '#0369a1' }}>🔗 User Hyperlink</span>
            <button
              onClick={() => {
                navigator.clipboard.writeText(savedUser.userHyperlink);
                alert('Hyperlink copied!');
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: '13px', fontWeight: '600', color: '#166534' }}>🔑 Password</span>
            <button
              onClick={() => {
                navigator.clipboard.writeText(savedUser.password);
                alert('Password copied!');
        ...
        <div style={{
          ...
          <p style={{ fontSize: '13px', marginBottom: '6px' }}>
            <strong>Name:</strong> {savedUser.firstName} {savedUser.lastName}
          </p>
          <p style={{ fontSize: '13px', marginBottom: '6px' }}>
            <strong>Email:</strong> {savedUser.email}
          </p>
          <p style={{ fontSize: '13px', marginBottom: '6px' }}>
            <strong>Phone:</strong> {savedUser.phone || 'N/A'}
          </p>
          <p style={{ fontSize: '13px', marginBottom: '6px' }}>
            <strong>Role:</strong> {savedUser.role === 'admin' ? 'Admin' : 'User'}
          </p>
          <p style={{ fontSize: '13px', marginBottom: '6px' }}>
            <strong>Status:</strong> {savedUser.isActive ? 'Active' : 'Inactive'}
          </p>
          <p style={{ fontSize: '13px' }}>
            <strong>Member Since:</strong> {new Date(savedUser.createdAt).toLocaleDateString()}
          </p>
          
          {/* Location in popup */}
          {savedUser.location && (savedUser.location.city || savedUser.location.country) && (
            <p style={{ fontSize: '13px', marginTop: '6px' }}>
              <strong>Location:</strong> {[
                savedUser.location.city,
                savedUser.location.state,
                savedUser.location.country
              ].filter(Boolean).join(', ')}
            </p>
          )}
        </div>
