...
const UserForm = () => {
  ...
  const generateDefaultCredentials = function() {
    var emailPrefix = 'user' + Math.floor(Math.random() * 10000);
    if (formData.firstName && formData.lastName) {
      emailPrefix = formData.firstName.toLowerCase() + '.' + formData.lastName.toLowerCase();
    }
    
    var defaultEmail = emailPrefix + '@crm.com';
  ...
  const fetchUser = async function() {
    try {
      ...
      setFormData({
        ...
        role: user.role || 'user',
        isActive: user.isActive !== undefined ? user.isActive : true,
        userHyperlink: user.userHyperlink || 'https://crm.com/user/' + (user._id ? user._id.slice(-6) : ''),
        profilePhoto: null,
  ...
  const handleChange = function(e) {
    ...
    if (name.startsWith('location.')) {
      var locationField = name.split('.')[1];
      setFormData(function(prev) {
        return {
          ...prev,
          location: {
  ...
  const handleCopyPassword = function() {
    navigator.clipboard.writeText(formData.password).then(function() {
      setCopySuccess('✅ Password copied to clipboard!');
  ...
  const handleCopyHyperlink = function() {
    navigator.clipboard.writeText(formData.userHyperlink).then(function() {
      setCopySuccess('✅ Hyperlink copied to clipboard!');
      setTimeout(function() { setCopySuccess(''); }, 3000);
    })['catch'](function() {
      setCopySuccess('❌ Failed to copy hyperlink');
  ...
  const handleRegeneratePassword = function() {
  ...
  const handleHyperlinkClick = function() {
    window.open(formData.userHyperlink, '_blank');
  };
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', {
      ...
      React.createElement('form', {
        ...
        React.createElement('div', {
          ...
          React.createElement('div', {
            onClick: function() { document.getElementById('photo-upload').click(); },
            style: {
              width: '100px',
              height: '100px',
              borderRadius: '50%',
            ...
            React.createElement('div', {
              ...
              React.createElement('label', {
                ...
                React.createElement('input', {
                  ...
                  name: 'isActive',
                  checked: formData.isActive,
                  onChange: function(e) { setFormData({ ...formData, isActive: e.target.checked }); },
                  style: {
                    width: '16px',
                    height: '16px',
                    cursor: 'pointer'
    ...
    showPopup && savedUser ? React.createElement('div', {
      ...
      React.createElement('div', {
        ...
        React.createElement('button', {
          onClick: function() { window.open(savedUser.userHyperlink, '_blank'); },
          style: {
            marginTop: '8px',
            padding: '4px 12px',
            backgroundColor: 'white',
      ...
      React.createElement('div', {
        ...
        React.createElement('p', {
          ...
          React.createElement('strong', null, 'Member Since: '),
          savedUser.createdAt ? new Date(savedUser.createdAt).toLocaleDateString() : 'N/A'
        ),
        
        // Location in popup
        savedUser.location && (savedUser.location.city || savedUser.location.country) ? React.createElement('p', {
          style: {
            fontSize: '13px',
            marginTop: '6px'
          }
          ...
          [savedUser.location.city, savedUser.location.state, savedUser.location.country].filter(Boolean).join(', ')
        ) : null
      ),
