...
const AdminMessages = () => {
  ...
  const [error, setError] = useState('');
  const [authError, setAuthError] = useState(false);
  const [showAllUsers, setShowAllUsers] = useState(true); // Show all users by default
  const messagesEndRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (adminUser?._id) {
      loadAllUsers(); // Load all users first
      loadConversations();
    }
  }, [adminUser]);
  ...
  const loadAllUsers = async () => {
    try {
      ...
      console.log('All users loaded:', filteredUsers.length);
      setUsers(filteredUsers);
    } catch (error) {
      console.error('Failed to load users:', error);
      if (error.response?.status === 401) {
  ...
  // Toggle between showing all users and conversations
  const toggleView = () => {
    ...
    React.createElement('div', {
      ...
      React.createElement('div', {
        ...
        loadingUsers ?
          ...
          filteredUsers.map((user) => {
            const fullName = `${user.firstName || ''} ${user.lastName || ''}`.trim() || 'Unknown User';
            // Find if this user has any unread messages
            ...
            return React.createElement('div', {
              ...
              style: {
                ...
                transition: 'background-color 0.2s',
                position: 'relative',
                border: selectedUser?._id === user._id ? '1px solid #3b82f6' : '1px solid transparent'
              },
              onMouseEnter: (e) => {
                if (selectedUser?._id !== user._id) {
                  e.currentTarget.style.backgroundColor = '#f9fafb';
